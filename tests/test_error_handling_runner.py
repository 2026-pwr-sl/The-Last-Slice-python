...
from sorting_functions import bubble_sort, insertion_sort, selection_sort
...
def test_team_module():
    ...
    members = []
    added, _message = add_member(members, "Henrique", "henrique111")
...
def test_sorting_functions():
    ...
    try:
        ...
        print(
            f"Test 1 - Bubble sort valid input: {'PASS' if result == [1, 2, 5, 8] else 'FAIL'}"
        )
    except Exception as error:
        print(f"Test 1 - FAIL: {error}")

    # Test 2: Invalid input (not a list)
    try:
        bubble_sort("not a list")
        print("Test 2 - Should have raised TypeError: FAIL")
    except TypeError as error:
        print("Test 2 - Invalid input caught: PASS")
        print(f"  Error: {error}")
...
def test_utils():
    ...
    # Test 2: Valid options requires user input, so skip in automated run
    print("Test 2 - Valid options: SKIPPED (requires user input)")
